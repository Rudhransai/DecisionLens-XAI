...
function addHeader() {
  ...
  doc.text('DECISIONLENS XAI · AIML 02 · HACKATHON & VIVA HANDBOOK', margin, 32);
...
function addFooter(pageNum, totalPages) {
  ...
  doc.setFontSize(8);
  doc.setTextColor(140, 150, 160);
  doc.text('DecisionLens XAI — Explainable AI Decision Support System', margin, pageHeight - 22);
...
function printParagraph(text, fontStyle = 'normal', color = [40, 50, 60], fontSize = 9) {
...
function printQuoteBox(text, lead = 'Say this first, in one sentence:') {
  ...
  const boxHeight = textLines.length * 12 + 24;
...
function printTable(headers, rows, colWidths) {
  checkPageBreak(25 + rows.length * 16);
  const startX = margin;
  ...
  rows.forEach((row, rIdx) => {
    ...
    row.forEach((cell, cIdx) => {
      ...
      doc.text(String(cell), cellX, y + 11);
      cellX += colWidths[cIdx];
    });
    doc.setDrawColor(235, 240, 245);
    doc.line(startX, y + 16, startX + contentWidth, y + 16);
    y += 16;
  });
  y += 8;
}
...
function printQA(q, a, tag = '') {
  ...
  printParagraph(a, 'normal', [50, 65, 75], 8.5);
  y += 4;
}
...
doc.setFontSize(8.5);
doc.setTextColor(13, 148, 136);
doc.text('FINAL YEAR & HACKATHON PROJECT · AIML 02 TRACK', margin, y);
y += 16;
...
y += 13;
doc.text('and the answer to every question an examiner or judge is likely to ask.', margin, y);
y += 18;
...
doc.setFont('helvetica', 'normal');
doc.text('AIML 02 — Explainable AI Decision Support System', margin + 45, y + 14);

doc.setFont('helvetica', 'bold');
doc.text('GitHub Repo:', margin + 10, y + 26);
doc.setFont('helvetica', 'normal');
doc.setTextColor(13, 148, 136);
doc.text('https://github.com/Rudhransai/DecisionLens-XAI', margin + 75, y + 26);
y += 48;
...
doc.text('Contents', margin + 12, y + 14);
...
const tocCol2 = [
  ...
  '6 · Measured Results & Calibration',
...
tocCol1.forEach((item, i) => {
  doc.text(item, margin + 12, y + 28 + i * 11);
});
tocCol2.forEach((item, i) => {
  doc.text(item, margin + contentWidth / 2 + 5, y + 28 + i * 11);
});
y += 90;

// Section 1: What It Is
printH1('1 · What It Is');
printParagraph('The one-paragraph answer, and the sentence to open with:');
...
printParagraph('Every requirement specified in AIML 02 is satisfied with 100% precision:');
